...
EOF
   	...
   	then
			...
			#creating a loop for looping through schemas that need to be backed up
			schemas="${schema1}"
...
database_import()
{
	...
	do
		...
		dest_path=${dest_path_dmp}

		#calling the secure copy function for the .tar file
		secure_copy_sts

		ssh -i /home/oracle/scripts/practicedir_isa_sep23/MyEC2KeyPair.pem ${user_acnt}@${dest_server} "ls -ltr ${dest_path_dmp}"

		#remotely extracting the .dmp and log file from the zipped file on the cloud server
		ssh -i /home/oracle/scripts/practicedir_isa_sep23/MyEC2KeyPair.pem ${user_acnt}@${dest_server} "cd ${dest_path_dmp} && tar -xvf ${l_schema}_${runner_db}_${TS}.tar"

#		ssh -i /home/oracle/scripts/practicedir_isa_sep23/MyEC2KeyPair.pem ${user_acnt}@${dest_server} "ls -ltr ${dest_path_dmp}/*isaac*"
		...
		ssh -i /home/oracle/scripts/practicedir_isa_sep23/MyEC2KeyPair.pem ${user_acnt}@${dest_server} "rm -r ${dest_path_dmp}/${l_schema}_${runner_db}_${TS}.dmp"
		#checking exit status
		action="dump file removal"
		check_exit_status

		ssh -i /home/oracle/scripts/practicedir_isa_sep23/MyEC2KeyPair.pem ${user_acnt}@${dest_server} "rm -r ${dest_path_dmp}/${l_schema}_${runner_db}_${TS}.log"
		#checking exit status
		action="log file removal"
		check_exit_status

		ssh -i /home/oracle/scripts/practicedir_isa_sep23/MyEC2KeyPair.pem ${user_acnt}@${dest_server} "rm -r ${dest_path_dmp}/${l_schema}_${runner_db}_${TS}.tar"
		#checking exit status
