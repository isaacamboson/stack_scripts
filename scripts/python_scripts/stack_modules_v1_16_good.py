...
def db_connection(**db_conn):
	...
	if db_conn["OP_NAME"].lower() == "export" and db_conn["OP_STATUS"] == "BEGIN":
		...
		cursor.execute("""insert into prod_activities (OP_ID, MON_EMAIL) select OP_ID, MONITORING_EMAIL from prod_operations where op_name = 'Export'""")
		cursor.execute("""update prod_activities set OP_STARTTIME = :OP_STARTTIME_INS, RUNNER = :RUNNER_INS, STATUS = :STATUS_INS where OP_ID = 1""", 
	...
	elif db_conn["OP_NAME"].lower() == "export" and db_conn["OP_STATUS"] == "COMPLETE":
		...
		cursor = connection.cursor()
		cursor.execute("""update prod_activities set OP_ENDTIME = :OP_ENDTIME_INS where OP_ID = 1""", OP_ENDTIME_INS = OP_ENDTIME)
		cursor.execute("""update prod_activities set STATUS = :STATUS_INS where OP_ID = 1""", STATUS_INS = db_conn["OP_STATUS"])
	...
	elif db_conn["OP_NAME"].lower() == "import" and db_conn["OP_STATUS"] == "BEGIN":
		...
		print(connection.version)
		cursor = connection.cursor()
		cursor.execute("""insert into prod_activities (OP_ID, MON_EMAIL) select OP_ID, MONITORING_EMAIL from prod_operations where op_name = 'Import'""")
		cursor.execute("""update prod_activities set OP_STARTTIME = :OP_STARTTIME_INS, RUNNER = :RUNNER_INS, STATUS = :STATUS_INS where OP_ID = 2""", 
				 OP_STARTTIME_INS = OP_STARTTIME, 
				 RUNNER_INS = db_conn["runner"], 
				 STATUS_INS = db_conn["OP_STATUS"]
				 )
	...
	elif db_conn["OP_NAME"].lower() == "import" and db_conn["OP_STATUS"] == "COMPLETE":
		...
		cursor = connection.cursor()
		cursor.execute("""update prod_activities set OP_ENDTIME = :OP_ENDTIME_INS where OP_ID = 2""", OP_ENDTIME_INS = OP_ENDTIME)
		cursor.execute("""update prod_activities set STATUS = :STATUS_INS where OP_ID = 2""", STATUS_INS = db_conn["OP_STATUS"])
		cursor.execute("""select po.op_id, po.op_name, po.op_type, pa.op_starttime, pa.op_endtime, pa.mon_email
						from prod_operations po
						left join prod_activities pa
						on po.op_id = pa.op_id
	...
	elif db_conn["OP_NAME"].lower() == "data_migration" and db_conn["OP_STATUS"] == "BEGIN":
		...
		print(connection.version)
		cursor = connection.cursor()
		cursor.execute("""insert into prod_activities (OP_ID, MON_EMAIL) select OP_ID, MONITORING_EMAIL from prod_operations where op_name = 'Data_Migration'""")
		cursor.execute("""update prod_activities set OP_STARTTIME = :OP_STARTTIME_INS, RUNNER = :RUNNER_INS, STATUS = :STATUS_INS where OP_ID = 3""", 
				 OP_STARTTIME_INS = OP_STARTTIME, 
				 RUNNER_INS = db_conn["runner"], 
				 STATUS_INS = db_conn["OP_STATUS"]
				 )
	...
	elif db_conn["OP_NAME"].lower() == "data_migration" and db_conn["OP_STATUS"] == "COMPLETE":
		...
		cursor = connection.cursor()
		cursor.execute("""update prod_activities set OP_ENDTIME = :OP_ENDTIME_INS where OP_ID = 3""", OP_ENDTIME_INS = OP_ENDTIME)
		cursor.execute("""update prod_activities set STATUS = :STATUS_INS where OP_ID = 3""", STATUS_INS = db_conn["OP_STATUS"])
		cursor.execute("""select po.op_id, po.op_name, po.op_type, pa.op_starttime, pa.op_endtime, pa.mon_email
						from prod_operations po
						left join prod_activities pa
						on po.op_id = pa.op_id
	...
	elif db_conn["OP_NAME"].lower() == "disk_monitoring" and db_conn["OP_STATUS"] == "BEGIN":
		...
		print(connection.version)
		cursor = connection.cursor()
		cursor.execute("""insert into prod_activities (OP_ID, MON_EMAIL) select OP_ID, MONITORING_EMAIL from prod_operations where op_name = 'Disk_Monitoring'""")
		cursor.execute("""update prod_activities set OP_STARTTIME = :OP_STARTTIME_INS, RUNNER = :RUNNER_INS, STATUS = :STATUS_INS where OP_ID = 5""", 
				 OP_STARTTIME_INS = OP_STARTTIME, 
				 RUNNER_INS = db_conn["runner"], 
				 STATUS_INS = db_conn["OP_STATUS"]
				 )
	...
	elif db_conn["OP_NAME"].lower() == "disk_monitoring" and db_conn["OP_STATUS"] == "COMPLETE":
		...
		cursor = connection.cursor()
		cursor.execute("""update prod_activities set OP_ENDTIME = :OP_ENDTIME_INS where OP_ID = 5""", OP_ENDTIME_INS = OP_ENDTIME)
		cursor.execute("""update prod_activities set STATUS = :STATUS_INS where OP_ID = 5""", STATUS_INS = db_conn["OP_STATUS"])
		cursor.execute("""select po.op_id, po.op_name, po.op_type, pa.op_starttime, pa.op_endtime, pa.mon_email
						from prod_operations po
						left join prod_activities pa
						on po.op_id = pa.op_id
...
def database_import(**imp_args):
	...
	try:
		...
		src = "{}/stack_temp_ISAAC_31012024093107.tar.gz".format(imp_args["backup_base"])
		...
		tarz_file = "{}/stack_temp_ISAAC_31012024093107.tar.gz".format(import_log_dir)
		...
		file_impar.write("dumpfile=stack_temp_ISAAC_31012024093107.dmp\n")
