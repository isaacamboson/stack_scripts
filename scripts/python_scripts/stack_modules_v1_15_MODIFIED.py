...
def db_connection(**db_conn):
	...
	if db_conn["OP_STATUS"] == "RUNNING":
		...
		timestring2 = datetime.datetime.now()
		ENDTIME = timestring2.strftime("%d-%b-%y %I.%M.%S.%f %p")
		OP_ENDTIME = ENDTIME.upper()
		print("First End Time: {}".format(OP_ENDTIME))

		connection = cx_Oracle.connect(user="STACK_ISA_SEP23", password="stackinc", dsn="MKIT-DEV-OEM/APEXDB")
		print(connection.version)

		cursor = connection.cursor()
		cursor.execute("""insert into operations values(:OP_ID_INS, :OP_NAME_INS, :OP_STARTTIME_INS, :OP_ENDTIME_INS, :RUNNER_INS, :STATUS_INS, :OP_TYPE_INS)""", OP_ID_INS = db_conn["OP_ID"], OP_NAME_INS = db_conn["OP_NAME"], OP_STARTTIME_INS = OP_STARTTIME, OP_ENDTIME_INS = OP_ENDTIME, RUNNER_INS = db_conn["runner"], STATUS_INS = db_conn["OP_STATUS"], OP_TYPE_INS = db_conn["OP_TYPE"])
		...
		print("Second End Time: {}".format(OP_ENDTIME))
...
def database_import(**imp_args):
	...
	#set as completed because when combined with db_export during db_migration
	#the 'insert' SQL statement block of db_connection needs to be skipped, and the 
	#'update' SQL statement block of db_connection needs to run instead.
